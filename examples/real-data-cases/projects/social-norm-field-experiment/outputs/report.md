...
`prepare_data.py` and `analyze.py` reproduce the public report from three committed, hash-locked, non-identifying snapshots. The treatment aggregate and household-clustered estimates are first recreated from the terms-governed participant file with `python3 scripts/build_tailored_source_snapshots.py social --social-csv /absolute/path/to/reviewed-file.csv --accept-isps-terms`; participant rows are never committed to the repository.

...
*Real-estate finance and market screening · Pre-acquisition evidence collection only · 10,000 modeled simulations*

## Executive Summary

- **Provisional preference — Brooklyn.** It is the highest-ranked feasible option, with decision value score **74.9/100** and a modeled **100% probability of being best among decision-feasible alternatives**.
- **The lead is meaningful rather than absolute.** It leads the next feasible option, Queens, by **0.089 utility points**.
...
- **Parameter lineage.** 66/66 governed parameters have a resolved source and approval-chain reference.
...
**Decision question:** Which borough market should advance first to property-level commercial real-estate diligence?
...
## Brooklyn leads on balanced value, not every dimension

**The preferred option earns its position through Public transaction depth.** The comparison still exposes a trade-off on **Relative price dispersion**, so the decision should be presented as a transparent compromise rather than a universal optimum.
...
**The decision is therefore driven by an explicit value model.** A stakeholder who places substantially more weight on Relative price dispersion may reasonably prefer another option; the two-sided weight-sensitivity section tests that possibility directly. The preferred option clips **0.0%** of criterion draws at the declared reference-scale bounds.

## Downside risk remains visible behind the average

**Brooklyn has expected utility 0.759, but its worst-decile average falls to 0.721.** The widest criterion-level uncertainty for this option is associated with **Approximate geography coverage**, making it a priority for further evidence collection.
...
**The declared factor model gives Brooklyn P(best) 100%, versus 100% under independent residuals and 100% under the stronger correlation stress.** Its CVaR10 moves from 0.721 independently to 0.721 under declared dependence and 0.721 under stress.
...
**The weakest modeled environment is Adverse transfer to a new setting, where the preferred option's risk-adjusted utility is 0.721.** The same feasible alternative remains ahead in every modeled scenario.
...
2. **Reduce uncertainty in Approximate geography coverage.** Validate or replace the widest uncertainty input using experimental, quasi-experimental, observational, or engineering evidence appropriate to the domain.
...
|---:|---|:---:|---:|---:|---:|---:|---:|---|:---:|
| 1 | Brooklyn | Yes | 74.9 | 0.759 | 0.721 | 100.0% | 100.0% | 0/10,000 observed; U95 0.00%; declared support excludes breach | Yes |
| 2 | Queens | Yes | 66.0 | 0.668 | 0.637 | 0.0% | 0.0% | 0/10,000 observed; U95 0.00%; declared support excludes breach | Yes |
| 3 | Manhattan | Yes | 56.1 | 0.568 | 0.539 | 0.0% | 0.0% | 0/10,000 observed; U95 0.00%; declared support excludes breach | No |
| 4 | Baseline: Bronx | Yes | 53.1 | 0.537 | 0.512 | 0.0% | 0.0% | 0/10,000 observed; U95 0.00%; declared support excludes breach | No |
| 5 | Staten Island | Yes | 51.3 | 0.519 | 0.496 | 0.0% | 0.0% | 0/10,000 observed; U95 0.00%; declared support excludes breach | No |
...
|---|---:|---:|---:|---:|---:|
| Public transaction depth | 45.0% | 0.385 share | 0.361 share–0.393 share | 0.641 | 0.0% |
| Relative price dispersion | 30.0% | 0.375 MAD/median | 0.368 MAD/median–0.397 MAD/median | 0.750 | 0.0% |
| Approximate geography coverage | 25.0% | 0.98 share | 0.92 share–1 share | 0.980 | 0.0% |

#### Queens

| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Public transaction depth | 45.0% | 0.253 share | 0.238 share–0.258 share | 0.422 | 0.0% |
| Relative price dispersion | 30.0% | 0.336 MAD/median | 0.329 MAD/median–0.356 MAD/median | 0.776 | 0.0% |
| Approximate geography coverage | 25.0% | 0.98 share | 0.92 share–1 share | 0.980 | 0.0% |
...
| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Public transaction depth | 45.0% | 0.172 share | 0.161 share–0.175 share | 0.286 | 0.0% |
| Relative price dispersion | 30.0% | 0.528 MAD/median | 0.518 MAD/median–0.559 MAD/median | 0.648 | 0.0% |
| Approximate geography coverage | 25.0% | 0.98 share | 0.92 share–1 share | 0.980 | 0.0% |
...
| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Public transaction depth | 45.0% | 0.105 share | 0.099 share–0.107 share | 0.175 | 0.0% |
| Relative price dispersion | 30.0% | 0.433 MAD/median | 0.425 MAD/median–0.458 MAD/median | 0.711 | 0.0% |
| Approximate geography coverage | 25.0% | 0.98 share | 0.92 share–1 share | 0.980 | 0.0% |
...
| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Public transaction depth | 45.0% | 0.065 share | 0.061 share–0.067 share | 0.109 | 0.0% |
| Relative price dispersion | 30.0% | 0.376 MAD/median | 0.369 MAD/median–0.399 MAD/median | 0.749 | 0.0% |
| Approximate geography coverage | 25.0% | 0.98 share | 0.92 share–1 share | 0.980 | 0.0% |
...
|---|---|---:|---:|---:|
| Independent residuals | Brooklyn | 100.0% | 0.721 | 0.00% |
| Declared factor model | Brooklyn | 100.0% | 0.721 | 0.00% |
| Loading stress ×1.35 | Brooklyn | 100.0% | 0.721 | 0.00% |

### Parameter provenance and approval

Coverage: **66/66 parameters sourced** and **66/66 approved for the declared use**. Every expanded JSON path is recorded in `decision-results.json` under `parameter_provenance.records`.

| Source ID | Source type | Owner | Approved uses | Approval chain |
|---|---|---|---|---|
| `commercial-real-estate-risk-analysis-output` | reproducible_project_output | Original publisher and repository analysis author | exploratory | 1. model_author (approved) |
...
- New York City Department of Finance, NYC Citywide Annualized Calendar Sales Update
- projects/commercial-real-estate-risk/outputs/results.json
...
- Case SHA-256: `0024c4e85aea39c0edfd1cb29fa44e72b388662dfa68435e95dcd33251e9a3b7`

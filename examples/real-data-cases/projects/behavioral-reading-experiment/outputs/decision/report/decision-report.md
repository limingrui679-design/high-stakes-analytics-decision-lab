...
- **Provisional preference — Baseline: Meaningful Only.** It is the highest-ranked feasible option, with decision value score **50.0/100** and a modeled **51% probability of being best among decision-feasible alternatives**.
...
- **Shared-shock sensitivity is explicit.** Relative to independent residuals, the declared factor model changes this option's P(best) by **+0.8%** and CVaR10 by **-0.001**; the ×1.35 loading stress does not change the modeled winner.
...
## Baseline: Meaningful Only leads on balanced value, not every dimension

**The preferred option earns its position through Assessment burden, Measurement stability.** The comparison still exposes a trade-off on **Standardized group separation**, so the decision should be presented as a transparent compromise rather than a universal optimum.
...
**The decision is therefore driven by an explicit value model.** A stakeholder who places substantially more weight on Standardized group separation may reasonably prefer another option; the two-sided weight-sensitivity section tests that possibility directly. The preferred option clips **0.0%** of criterion draws at the declared reference-scale bounds.

## Downside risk remains visible behind the average

**Baseline: Meaningful Only has expected utility 0.515, but its worst-decile average falls to 0.453.** The widest criterion-level uncertainty for this option is associated with **Standardized group separation**, making it a priority for further evidence collection.
...
**The declared factor model gives Baseline: Meaningful Only P(best) 51%, versus 50% under independent residuals and 51% under the stronger correlation stress.** Its CVaR10 moves from 0.454 independently to 0.453 under declared dependence and 0.453 under stress.
...
**The weakest modeled environment is Adverse transfer to a new setting, where the preferred option's risk-adjusted utility is 0.477.** The same feasible alternative remains ahead in every modeled scenario.
...
**The baseline choice survives 67% of local weight stresses.** The following weight perturbation changes the winner: ↑ Standardized group separation → Pseudoword Only; ↓ Assessment burden → Pseudoword Only.
...
2. **Reduce uncertainty in Standardized group separation.** Validate or replace the widest uncertainty input using experimental, quasi-experimental, observational, or engineering evidence appropriate to the domain.
...
|---:|---|:---:|---:|---:|---:|---:|---:|---|:---:|
| 1 | Baseline: Meaningful Only | Yes | 50.0 | 0.515 | 0.453 | 51.1% | 51.1% | 0/10,000 observed; U95 0.00%; declared support excludes breach | Yes |
| 2 | Pseudoword Only | Yes | 49.4 | 0.515 | 0.429 | 47.5% | 47.5% | 0/10,000 observed; U95 0.00%; declared support excludes breach | Yes |
| 3 | Combined Protocol | Yes | 35.1 | 0.379 | 0.266 | 1.3% | 1.3% | 0/10,000 observed; U95 0.00%; declared support excludes breach | Yes |
...
#### Baseline: Meaningful Only

| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Standardized group separation | 45.0% | 0.247 standardized difference | 0.024 standardized difference–0.579 standardized difference | 0.123 | 0.0% |
| Assessment burden | 30.0% | 226.7 mean fixation duration | 215.7 mean fixation duration–244.1 mean fixation duration | 0.830 | 0.0% |
...
| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Standardized group separation | 45.0% | 0.382 standardized difference | 0.03 standardized difference–0.815 standardized difference | 0.191 | 0.0% |
| Assessment burden | 30.0% | 261.8 mean fixation duration | 248.1 mean fixation duration–282.6 mean fixation duration | 0.752 | 0.0% |
| Measurement stability | 25.0% | 0.814 index | 0.748 index–0.866 index | 0.814 | 0.0% |

#### Combined Protocol

| Criterion | Weight | Mean | P05–P95 | Normalized score | Scale clipping |
|---|---:|---:|---:|---:|---:|
| Standardized group separation | 45.0% | 0.598 standardized difference | 0.157 standardized difference–1.054 standardized difference | 0.299 | 0.0% |
| Assessment burden | 30.0% | 488.4 mean fixation duration | 464.4 mean fixation duration–528.1 mean fixation duration | 0.248 | 0.0% |
| Measurement stability | 25.0% | 0.68 index | 0.616 index–0.729 index | 0.680 | 0.0% |

### Correlation sensitivity

| Dependence state | Modeled winner | Recommended option P(best) | CVaR10 | Breach U95 |
|---|---|---:|---:|---:|
| Independent residuals | Baseline: Meaningful Only | 50.3% | 0.454 | 0.00% |
| Declared factor model | Baseline: Meaningful Only | 51.1% | 0.453 | 0.00% |
| Loading stress ×1.35 | Baseline: Meaningful Only | 51.2% | 0.453 | 0.00% |
...
|---|---|---|---|---|
| `behavioral-reading-experiment-analysis-output` | reproducible_project_output | Original publisher and repository analysis author | exploratory | 1. model_author (approved) |

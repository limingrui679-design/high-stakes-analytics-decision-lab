...
def analyze_spatial_transit(project_root: Path):
    ...
    result["headline_metrics"] = [f"Analyzed Massachusetts tracts: {result['data']['tracts_analyzed']:,}", f"Composite-need complete cases: {result['data']['composite_need_complete_case_tracts']:,}; missing proxy: {result['data']['rent_to_income_proxy_missing_tracts']:,}", f"MBTA rapid-transit stop records: {len(stops):,}", f"High-poverty weighted nearest-stop distance: {high_distance:.2f} km", "Site decision: blocked pending land and feasibility evidence"]
    result.setdefault("decision_support", {})["reversal_conditions"] = ["Parcel and zoning evidence makes a selected tract infeasible.", "Network travel times reverse straight-line access rankings.", "Stakeholder-defined equity priorities change the need weights.", "A reviewed missing-data route changes the complete-case hub set."]
    return result
...
REPORT_COPY.update(
    {
        ...
        "spatial-equity-planning": {"answer": "ACS need patterns and observed MBTA stop proximity can prioritize local review, but no site recommendation is released until parcel, zoning, network, cost, and stakeholder evidence is supplied.", "findings": [], "methods": "ACS tract indicators, complete-case composite need with median-imputation and legacy-route audits, spatial autocorrelation, heuristic five-hub allocation, bootstrap/sensitivity checks, and population-weighted nearest-MBTA-stop distance.", "limits": "Composite need excludes missing rent-to-income proxies while retaining those tracts for non-composite analysis; tract internal points and straight-line distance remain planning screens, and land, zoning, networks, capacity, cost, and community consent are missing."},
    }
)
